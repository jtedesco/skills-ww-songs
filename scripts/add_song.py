...
Steps:
  ...
  2. Auto-fetch metadata from MusicBrainz (year, album, genre, mood).
  3. Interactively prompt for manual fields (key, BPM, vocalist, etc.).
  4. Append the new row to songs_metadata.csv.
  5. Audit the full song list and print a summary.
"""
...
# ---------------------------------------------------------------------------
# Helpers — shared with enrich_metadata.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 3 — Interactive prompts for manual fields
# ---------------------------------------------------------------------------
...
def audit_song_list(songs: list):
    ...
    # --- Summary ---
    issues = len(not_ready) + len(missing)
    print(f"\n{'=' * 60}")
    if issues == 0:
