...
**Everything else shares pages naturally**: only `SET N`/`ENCORES` headings get the forced page break above — `render_pdf.py`'s `wrap_set_blocks()` checks each `<h2>`'s text and leaves every other section (the trailing `## GIG SUMMARY` / `## SONGS IN PROGRESS` pages, or the older separate `## SONGS NOT SELECTED` / `## ARCHIVED SONGS` format) unwrapped, so the browser's normal pagination lets short trailing sections share a page instead of each claiming its own mostly-empty one. Don't widen that regex match back to "every h2" — that was the previous (bugged) behavior this section is describing the fix for.
