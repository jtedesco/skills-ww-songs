...
It is deliberately scoped to that one page — the set tables, the YouTube playlists, the gig summary and its Repertoire Not Scheduled table all keep the full title, so an abbreviation can't leak into a surface where nobody would recognise it. `render_floor_sheet_lines()` in `build_setlist.py` reads it directly for CSV-sourced songs; `apply_substitution.py` passes its own resolver, because its song dicts are parsed back out of the printed `.md` table and don't carry the column.
...
Two values describe what a song *is*. They print **merged into one `Genre` cell** — `Yacht Rock / Jazz Rock` — in the set tables and in the GIG SUMMARY page's Repertoire Not Scheduled table, via `genre_subgenre_display_string()`. When the two read the same, or either is blank, the cell prints just one (`Alternative Rock`, not `Alternative Rock / Alternative Rock`).

| Column | Stored? | Source | Example |
| :--- | :--- | :--- | :--- |
| **Genre** | yes, `genre` | hand-curated | `Yacht Rock`, `Classic Rock`, `Motown`, `Disco` |
| **Subgenre** | no — derived | `musicbrainz_genre` | `Soft Rock`, `Jazz Rock`, `Synth-Pop` |

**`genre` is free text.** Whatever the column holds is what prints, via `genre_display_string()`. It was a closed vocabulary (`Yacht Rock` / `Classic Rock`, everything else blank) while its only job was separating the yacht pool from the rest — it is not one now that every song carries a genre. **Don't reintroduce a whitelist**: a genre the band types would silently print as an empty cell.

**A song outside the yacht pool takes its Genre from its Subgenre**, so the two often read the same — which is why the merged cell collapses a duplicate. That is expected, not a bug — Genre is stored and editable, Subgenre tracks MusicBrainz and will change under you when `enrich_metadata.py` runs.

**The yacht pool is a separate decision from the genre list.** `--gig-type yacht` draws from `YACHT_POOL_VALUES`, which names `Yacht Rock` and `Classic Rock` explicitly. Deriving it from the column's contents would book a Synth-Pop or Motown song into a yacht gig the moment it was tagged. Adding a genre and making it yacht-eligible are two decisions; only the second one touches that set.

**Two legacy spellings still map.** The column was once `yacht_adjacent`, holding `Yes` and `Adjacent`. `GENRE_LEGACY_SPELLINGS` in `build_setlist.py` still maps both, because the CSV lives in a shared Drive folder where a hand-edited cell or a row pasted from an older copy can reintroduce one. Don't "clean up" those two entries. A wholesale restore of a pre-rename CSV is a different, louder problem: the column itself would be back to `yacht_adjacent`, which `test_database_integrity()`'s required-fields check fails on immediately — deliberately, so don't add a fallback that quietly accepts the old column name.

## Decade Column

**Decade** prints as `1970s` / `2010s`, derived by `decade_display_string()` from `release_year` and blank when that is empty. Like Subgenre it is **derived at render time, not stored** — a `decade` column would be a second copy of `release_year`, free to drift from it, and placing a song in its decade is exactly what `release_year` already exists for. To correct a Decade, edit the year; to correct a Subgenre, edit that song's `musicbrainz_genre`.

**`subgenre_display_string()` filters the tag list.** MusicBrainz tags are a community free-for-all, and `musicbrainz_genre` stores the raw top three. Dropped before picking the first survivor: anything with a four-digit year or a decade in it (`2008 Universal Fire Victim`, `Late 60'S Early 70'S`), a short list of non-subgenre community tags, and **`Rock`** — which is `enrich_metadata.py`'s literal fallback string when a lookup finds nothing usable, so a stored `Rock` always means "no data" and a blank cell says so honestly.
...
`SUBGENRE_FIXUPS` also normalises MusicBrainz's print spellings, including the two it ships for the same thing (`Singer/Songwriter` → `Singer-Songwriter`).
...
- It still enforces the same constraints `build_setlist.py` does, even though it isn't re-solving the whole setlist:
  ...
  - **Repertoire Not Scheduled**: the trailing GIG SUMMARY page is always fully regenerated from the final scheduled songs, so a swap correctly moves songs between the set table and the not-scheduled groups.
- `--set-length "Title" "M:SS"` overrides a song's *performed* length for that gig only. It also doubles as the way to force a full re-render of the regenerated tail (floor sheet, gig summary) onto a setlist written before one of those sections existed: re-set a song to the length it already has, and the document rebuilds with no content change. `--refresh-intro-notes` is **not** the way to do this — it overwrites every printed Intro cell from the database, silently dropping gig-specific staging ("last song of Set 1") and re-introducing notes for absent members.
...
**`GIG SUMMARY` is the last page, in two columns**: `render_pdf.py`'s `wrap_set_blocks()` forces the same page break for `## GIG SUMMARY` as `SET N`/`ENCORES` and adds a `.summary-page` class that lays it out with `column-count: 2`. Stats and the vocalist breakdown sit at the top of the left column; the Repertoire Not Scheduled table flows down the left column and on into the right. Chrome fragments a table row-by-row across columns and repeats its `<thead>` at the top of the second, so this is one table, not two halves split in Python. Trailing sections on older setlists (`## SONGS IN PROGRESS`, or the separate `## SONGS NOT SELECTED` / `## ARCHIVED SONGS` format) are left unwrapped, so they flow naturally and share a page with whatever precedes them instead of claiming their own mostly-empty one. Don't widen the forced-break match back to "every h2" — that was the original (bugged) behavior this section is describing the fix for.
...
- Song's `genre` is in **`YACHT_POOL_VALUES`** (`Yacht Rock` / `Classic Rock`) **and** it is gig-ready → add its title to the `yacht_songs` set in `test_yacht_scenario()`, or Scenario 1 fails the moment the solver puts it in a yacht setlist. This one bites on a *readiness flip*, not just a new song: making an already-classified song gig-ready is what makes it eligible for the yacht pool.
...
- **Per-song substitution effects are already visible without prose.** A covered vocal prints in the Lead Vocal column as `Lauren (for David)`, and a song cut for an absent member simply isn't in the table (it shows up in GIG SUMMARY's Repertoire Not Scheduled table). The callout was re-describing what the table already showed.
...
- **Song table** with columns: `#`, `Title`, `Artist`, `Genre`, `Decade`, `Key`, `BPM`, `Length`, `Lead Vocal`, `Energy`, `Dance`, `Intro` (`Genre` is the merged Genre / Subgenre cell — see "Genre and Subgenre" above). The layout is defined once, as `TABLE_COLUMNS` in `build_setlist.py` — `apply_substitution.py` regenerates its tables from the same constant, so don't restate the column order anywhere else.
  - **The PDF uses fixed column widths.** `render_pdf.py` sets `table-layout: fixed` on `.song-table` with a percentage for every column, summing to 100%. Under auto layout the columns sized to their content and Intro — the one column meant to wrap — got whatever was left, a one-word strip. The rules are positional `nth-child()` selectors (`#` is 1, `Genre` 4, `Dance` 11, `Intro` 12); adding or reordering a column means re-dividing all twelve shares, not just moving one.
  - **A Genre cell breaks only at its ` / `**, and Energy only after its arrow (`Medium→` / `High`) — `keep_genre_parts_together()` in `render_pdf.py`. Letting `Yacht Rock / Jazz Rock` break anywhere stacked it three lines tall and cost rows per page; holding it to one line crowded Intro. A one-part genre (`Alternative Rock`) is left free to wrap.
- **Duration summary**: Music time, transitions, breaks, grand total
- **Gig Summary page**: The last section, `## GIG SUMMARY` (a fresh PDF page laid out in two columns — see PDF Export above), built by `render_summary_page_lines()` in `build_setlist.py`, combining three subsections:
  ...
  - `### Repertoire Not Scheduled` — ONE table (`| Song | Genre | Decade | Energy |`) of every song this gig didn't use, via `render_repertoire_lines()`, split into three groups by heading rows inside the table: **Active — Not Selected** (gig-ready, not archived), **In Progress** (not gig-ready, not archived) and **Archived**, each with its count. A song on tonight's setlist is in no group — including an in-progress song pulled in with `--include-not-ready`. An empty group keeps its heading and gets one italic placeholder row.
    - **Heading rows are marked with an invisible `<!--group-->` comment** in their first cell, so the `.md` stays a plain four-column table (three cells blank). `render_pdf.py`'s `style_repertoire_table()` turns each into one full-width heading cell, and every heading after the first gets a white gap above it, so In Progress and Archived don't read as one more song row.
    - **`style_repertoire_table()` must run after `apply_shading()`.** Shading rebuilds every `<tbody>` from its bare `<tr>` rows; a row that already carries a class is silently dropped — which took the In Progress and Archived headings with it the first time.
    - This replaced the two-sided `Not Selected / Archived` table and the separate `## SONGS IN PROGRESS` page. A short gig that leaves most of the repertoire unused can still run the page long; that's a content-volume limit, not a formatting bug.
...
- **Floor Sheet**: a single large-type page, `## FLOOR SHEET`, built by `render_floor_sheet_lines()` in `build_setlist.py` and placed after the last set (and after `## ENCORES`, if any) but before `## GIG SUMMARY`. It's the page the band tears off and tapes to the stage floor, so it's designed for one thing — being read at a glance from standing height:
  - **Only what's needed mid-song-change**: number, title, key, BPM, the intro cue, and an `[X]` on each set's emergency cut (with a one-line legend at the top, printed only when something is marked — the page gets read on its own, away from the set tables that spell out the 🛑 convention). No lead/backup vocals, no energy or dance marks, no emoji. Every extra field costs font size, and font size is the whole point of the page. Don't add columns to it "for completeness" — that information is already in the set tables two pages earlier.
  ...
  - **An intro note may carry its own markup** — e.g. a bold `**[Vamp]**` tag prefixed onto the printed Intro cell for one gig. Two things make that safe, and both matter if you change this area:
    - The cue is wrapped in a literal `<em>`, not `*...*`. A note that is *only* a bold tag would otherwise collapse to `***[Vamp]***`, which CommonMark parses as a **top-level** `<strong>` — and would then inherit the title tier's `display: block` at 15.5pt, printing the tag as a full-size line of its own.
    - The title rule is `.floor-sheet p > strong` (direct child), so a nested tag keeps the small tier's size; `.floor-sheet p em strong` re-asserts `display: inline`.
    - Anywhere a printed note gets quoted inside other prose — currently only the constraints table's Absent-Member Note Check row — run it through `_plain()` first, or the tag renders bold mid-sentence and pulls the eye away from what the constraint is reporting.
  - **A per-gig tag belongs in the setlist `.md`, not `songs_metadata.csv`.** The Intro cells in the set tables are preserved verbatim across revisions (that's why `--refresh-intro-notes` is opt-in), and the floor sheet re-reads them from those cells — so tagging the table is enough, and it stays scoped to that one gig. Putting it in the database would push it onto every setlist on the next regeneration.
  ...
  **Measuring which titles wrap** (repeat this after adding a long-titled song, or after any change to the floor-sheet type size — eyeballing a thumbnail is not reliable):
  ...
  - **One page is a hard requirement, and it's tight.** Current settings (15.5pt title, 2px inter-entry margin) fit a **37-song, 3-set gig** on one page with very little to spare. This is a real content ceiling, not a formatting bug: the 2026-07-18 Local Bar & Grill gig (39 songs across 3 sets *and* 2 acoustic breaks, so 5 block headings) needs two pages, and still does at 14.5pt — shrinking the type far enough to fit it would cost every other gig legibility for the sake of one. Leave it at two pages rather than degrading the common case.
...
* **Approach**:
  ...
  - Songs that require Martin's acoustic guitar (*Landslide*, *Blackbird*) are cut from the acoustic break pool when Martin is out. Songs with a Martin-out substitution (*Wish You Were Here*, *Ventura Highway*, *Ooh La La*) remain in the pool.
